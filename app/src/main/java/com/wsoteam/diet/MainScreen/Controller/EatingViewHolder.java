...
public class EatingViewHolder extends RecyclerView.ViewHolder {
  ...
  @BindView(R.id.layoutCommonInfo) ConstraintLayout layoutCommonInfo;
  ...
  private void remind(String eatingGroup) {
    ...
    phrases.add(
        "Вы сегодня " + eatingGroup.toLowerCase() + "али?\n Добавьте продукты, которые вы ели");
    phrases.add(
        "Занесите " + eatingGroup.toLowerCase() + " сейчас\n и достигните своей цели быстрее");
  ...
  private void setExpandableView() {
    if (eatingGroup.size() < 1) {
      ...
      ibtnOpenList.setOnClickListener(view -> {
        if (!isButtonPressed) {
          ibtnOpenList.setImageDrawable(
              DrawableUtilsKt.getVectorIcon(context, R.drawable.close_eating_list));
        } else {
          ibtnOpenList.setImageDrawable(
              DrawableUtilsKt.getVectorIcon(context, R.drawable.open_eating_list));
        }
        rvListOfFoodEatingCard.setAdapter(new InsideAdapter(eatingGroup,
            context, !isButtonPressed, getAdapterPosition(), this::refreshUI).setUpdateCallback(
            updateCallback));
